...
class UserProxyAgent(AgentBase):
    def __init__(self, prompt_registry, llm=None) -> None:
        super().__init__(agent_name="UserProxy", prompt_id="userproxy_v1", prompt_registry=prompt_registry, llm=llm)
    ...
    async def roleplay(
        ...
        """Generate the user-side roleplay message."""
        fallback_payload = {
            "action": {"type": ActionType.PROPOSE_OFFER.value, "payload": {}},
